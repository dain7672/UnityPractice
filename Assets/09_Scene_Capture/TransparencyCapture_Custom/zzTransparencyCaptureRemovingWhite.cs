...
public class zzTransparencyCaptureRemovingWhite
{
        ...
        var lPreBackgroundColor = lCamera.backgroundColor;
        {
            ...
            for (int x = 0; x < lWhiteBackgroundCapture.width; ++x)
            {
                for (int y = 0; y < lWhiteBackgroundCapture.height; ++y)
                {
                    ...
                    if (lColorWhenBlack == Color.black) {
                        lWhiteBackgroundCapture.SetPixel(x, y,Color.clear);
                    } else if (lColorWhenBlack != Color.clear)
                    {
                        //set real color
    ...
    static Color getColor(Color pColorWhenBlack,Color pColorWhenWhite)
    {
        ...
        // 색깔 소실의 문제가 있는 경우 아래와 같이 식을 변경하라.
